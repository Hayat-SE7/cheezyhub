...
test.describe('Kitchen Panel — Accessibility (WCAG 2.1 AA)', () => {
  ...
  test('kitchen dashboard has no a11y violations', async ({ page }) => {
  ...
  test('order cards are screen-reader friendly', async ({ page }) => {
    await page.goto('/kitchen');
    await page.waitForLoadState('networkidle');

    // Check that order-related content uses semantic HTML or ARIA
    const results = await new AxeBuilder({ page })
