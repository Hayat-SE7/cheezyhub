...
export default function DeliveryError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
      ...
      <p className="text-[#4a4a58] mb-6 max-w-sm text-sm">An unexpected error occurred in the delivery panel.</p>
      <div className="flex gap-3">
        ...
        <Link href="/delivery" className="px-6 py-3 rounded-xl bg-[#1e1e22] hover:bg-[#2a2a32] text-[#9898a5] font-semibold transition-colors">
          Back to Home
        </Link>
      </div>
    </div>
